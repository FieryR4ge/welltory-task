...
def validate(file, json_data, schema):
    ...
    for error in errors:
        info.append({
            'error': error.message,
            'filename': file,
            'schema_name': json_data['event'].replace(' ', ''),
            'error_info': recommendations(error.message)
            }
        )
        ...
        if json_data is not None and bool(json_data):
            ...
            if os.path.exists(schema_file):
                ...
                result.append({
                    ...
                    'schema_name': None,
                    'error_info': 'File schema for json not found'
                    }
                )
        else:
            result.append({
                ...
                'filename': os.path.basename(json_file),
                'schema_name': None,
                'error_info': 'Enter the correct format for the file'
                }
            )
